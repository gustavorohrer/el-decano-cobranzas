...
interface PaymentsManagementProps {
  ...
  onRegister: (months: number[]) => MemberPayment[];
  onClose: () => void;
}

const PaymentsManagement: React.FC<PaymentsManagementProps> = ({ member, payments, membershipRates, onRegister, onClose }) => {
  const [selectedMonths, setSelectedMonths] = React.useState<number[]>([]);
  const [lastRegisteredPayments, setLastRegisteredPayments] = React.useState<MemberPayment[]>([]);
  ...
  const isPaid = (month: number) => payments.some(p => p.month === month && p.year === currentYear && !p.is_annulled);

  const getRateAmount = (month: number) => {
    const c = membershipRates.find(cf => cf.month === month && cf.year === currentYear && cf.category === member.category);
    return c ? c.amount : 2500;
  };

  const totalSelected = selectedMonths.reduce((acc, month) => acc + getRateAmount(month), 0);

  const handleRegister = () => {
    const createdPayments = onRegister(selectedMonths);
    setLastRegisteredPayments(createdPayments);
    setSelectedMonths([]);
    setShowConfirmPayment(false);
    setShowSuccessModal(true); // Mostrar el prompt de impresión inmediatamente
  };

  const generateReceiptHTML = (payment: MemberPayment) => `
    <div class="receipt-box" style="border: 2px solid #064e3b; padding: 30px; border-radius: 20px; max-width: 600px; margin: 0 auto 40px auto; page-break-after: always;">
      <div class="header" style="display: flex; justify-content: space-between; border-bottom: 2px solid #f0fdf4; padding-bottom: 20px; margin-bottom: 20px;">
        ...
        <div style="text-align: right;">
          <p class="receipt-num" style="color: #666; font-size: 14px;">RECIBO N° <strong>${payment.receipt_number}</strong></p>
          <p style="font-size: 12px;">Fecha: ${new Date(payment.payment_date).toLocaleDateString()}</p>
        </div>
      </div>
      <div class="info-grid" style="display: grid; grid-template-cols: 1fr 1fr; gap: 20px; margin-bottom: 30px;">
        <div>
          ...
          <p class="value" style="font-size: 16px; font-weight: bold;">${member.first_name} ${member.last_name} (#${member.member_number})</p>
        </div>
        <div>
          <p class="label" style="font-size: 10px; font-weight: bold; text-transform: uppercase; color: #059669;">Concepto</p>
          <p class="value" style="font-size: 16px; font-weight: bold;">Cuota Social ${MONTHS[payment.month - 1]} / ${payment.year}</p>
        </div>
      </div>
      <div class="total" style="background: #f0fdf4; padding: 20px; border-radius: 15px; text-align: right;">
        ...
        <p class="total-val" style="font-size: 28px; font-weight: 900; color: #064e3b;">$${payment.amount.toLocaleString()}</p>
      </div>
  ...
  const printReceipt = (payment: MemberPayment) => {
    const printWindow = window.open('', '_blank');
    if (!printWindow) return;
    printWindow.document.write(`<html><head><title>Recibo ${payment.receipt_number}</title><style>body { font-family: sans-serif; padding: 40px; color: #333; }</style></head><body>${generateReceiptHTML(payment)}</body></html>`);
  ...
  const printAllReceipts = () => {
    const printWindow = window.open('', '_blank');
    if (!printWindow) return;
    printWindow.document.write(`<html><head><title>Recibos de Pago</title><style>body { font-family: sans-serif; padding: 40px; color: #333; }</style></head><body>${lastRegisteredPayments.map(p => generateReceiptHTML(p)).join('')}</body></html>`);
    printWindow.document.close();
    printWindow.print();
  ...
  return (
    <div className="bg-white/95 backdrop-blur-md rounded-3xl p-8 shadow-2xl border border-emerald-100 max-w-2xl w-full mx-auto animate-in zoom-in duration-200">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-black text-emerald-950">Registro de Cuotas {currentYear}</h2>
          <p className="text-emerald-700 font-medium italic">Socio: {member.first_name} {member.last_name} (#{member.member_number})</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
        {MONTHS.map((month, index) => {
          ...
          const amount = getRateAmount(monthIndex);
          ...
          return (
            <button
              ...
              className={`
                ...
                ${paid ? 'bg-emerald-50 border-emerald-200 text-emerald-700 opacity-80 cursor-default' : 
                  selected ? 'bg-emerald-700 border-emerald-800 text-white transform scale-105 z-10' : 
            ...
            >
              ...
              <span className={`text-sm font-bold ${selected ? 'text-emerald-100' : 'text-slate-900'}`}>${amount}</span>
              {paid ? <Check size={16} /> : selected ? <Clock size={16} /> : <div className="h-4" />}
            </button>
          );
        })}
      </div>
      ...
      <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100 flex items-center justify-between mb-6 shadow-inner">
        <div>
          <p className="text-xs font-bold text-emerald-800 uppercase tracking-widest">Total a Pagar ({selectedMonths.length} meses)</p>
          <p className="text-3xl font-black text-emerald-950">${totalSelected.toLocaleString()}</p>
        </div>
        <button
          ...
          className={`
            ...
            ${selectedMonths.length > 0 ? 'bg-emerald-700 text-white hover:bg-emerald-800 shadow-emerald-900/40 hover:-translate-y-1' : 'bg-slate-300 text-slate-500 cursor-not-allowed shadow-none'}
          `}
        >
          <FileText size={20} />
          Confirmar Pago
      ...
      <ConfirmationModal
        ...
        message={`¿Está seguro de registrar el cobro de ${selectedMonths.length} cuota(s) por un valor de $${totalSelected.toLocaleString()}?`}
